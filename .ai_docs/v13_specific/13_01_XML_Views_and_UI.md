# Odoo 13: XML Views and UI Standards

`<role_instruction>`

You are writing code for **Odoo 13.0**. You must strictly use the legacy Odoo XML view
...
```
<!-- FATAL ERROR IN V13: Using modern Python evaluation strings -->
...
```
<!-- GOOD: Odoo 13 attrs dictionary syntax -->
...
If visibility or read-only status depends purely on a field named `state`, you can and
should use the `states` attribute as a shortcut instead of `attrs`. Provide a
comma-separated list of the states where the field should be editable/visible.
...
In Odoo 13, Tree view decorations (e.g., `decoration-danger`, `decoration-info`) require
...
```
<!-- GOOD: Odoo 13 decoration domains -->
